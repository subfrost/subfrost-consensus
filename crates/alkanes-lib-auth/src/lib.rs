...
use alkanes_support::{
    cellpack::Cellpack, parcel::{AlkaneTransferParcel, AlkaneTransfer}, id::{AlkaneId}
};
use std::sync::{Arc};
...
pub trait AuthenticatedResponder : AlkaneResponder {
  fn deploy_auth_token(&self, units: u128) -> Result<AlkaneTransfer> {
    ...
    StoragePointer::from_keyword("/auth").set(Arc::new(<AlkaneId as Into<Vec<u8>>>::into(AlkaneId {
      block: 2,
      tx: sequence
    })));
  ...
  fn only_owner(&self) -> Result<()> {
    ...
    let response = self.call(&cellpack, &AlkaneTransferParcel(vec![AlkaneTransfer {
