...
pub async fn read_sample_contract() -> Result<Vec<u8>> {
    ...
    let wasm_binary_jsvalue = JsFuture::from(fetch_promise).await.map_err(|e| {
        anyhow!("Failed to fetch WASM file: {:?}", e.as_string().unwrap_or_else(|| "Unknown error".into()))
    })?;

    // Step 2: Convert the fetched value into a Response object
    let response: Response = wasm_binary_jsvalue.dyn_into().map_err(|e| {
        anyhow!("Failed to cast JsValue to Response: {:?}", e)
    })?;
    ...
    let wasm_bytes_jsvalue = JsFuture::from(response.array_buffer().map_err(|e| {
        anyhow!("Failed to get array buffer: {:?}", e.as_string().unwrap_or_else(|| "Unknown error".into()))
    })?).await.map_err(|e| {
        anyhow!("Failed to resolve array buffer future: {:?}", e.as_string().unwrap_or_else(|| "Unknown error".into()))
    })?;
